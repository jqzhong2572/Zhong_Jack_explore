...
explore2 <- function(data,plotswitch="off", threshold=0.5, bins=NULL){
  # This function work with defensive conditions of explore
  
  # parameter: same as explore
  # return: explore with fine parameter as inputs
  # take care of N/As
  data <- na.omit(data)
  ...
  while(plotswitch != "off" && plotswitch != "on" && plotswitch != "grid"){  
    ...
    plotswitch <- readline(prompt="Enter your option(off / on / grid): ")  #re-enter the input
  }
